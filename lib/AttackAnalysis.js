import { CharacterInfo, ShipInfo } from '../data/Store';
...
function getSpecialPlus(currentCharacterList, specialAbilityList) {
  ...
  const magni = specialAbilityList.map(character => {
    return character.specialAbility.plus;
  })
  .reduce((magni, fn) => {
    return magni * fn({
      character: characterInfo,
...
function getBasicAttack(attack, add, plus) {
  let basic = attack + add;
  for (let key in plus) {
    basic *= plus[key];
  }
...
export default function attackAnalysis({ enemy, ship, characters }) {
  ...
  characters.forEach(function(character) {
    ...
    // Basic Attack
    const addAttack = shipInfo.add({ character: characterInfo });
    const basic = getBasicAttack(character.attack, addAttack, plus);
