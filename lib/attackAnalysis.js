...
function getCharacterInfo(number) {
  let characterInfo = characterList[number];
  if (typeof characterInfo !== 'object') {
    characterInfo = new Character();
    characterInfo.no = number;
...
export default function attackAnalysis({ enemy, boat, characters }) {
  ...
  const captains = characters.reduce((list, character) => {
    if (character.captainEffect === true) {
      list.push(getCharacterInfo(character.no));
    }
    return list;
  }, []);
  ...
  const specialAbilities = characters.reduce((list, character) => {
    if (character.specialAbility === true && getCharacterInfo(character.no).specialAbility.magnification !== null) {
      list.push(getCharacterInfo(character.no));
    }
    return list;
  }, []);
  ...
  characters.forEach(function(character) {
    const characterInfo = getCharacterInfo(character.no);
