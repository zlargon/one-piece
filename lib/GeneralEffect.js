// TODO: find out why 'CharacterInfo' is always undefined here.
...
export function ClassEffect(className, magni, low = 1) {
  ...
  // check classes
  const validClasses = ['格鬥', '斬擊', '打擊', '射擊', '強化用', '進化用', '自由', '博識', '強韌'];
...
export function OrderEffect(typeOrder, magni) {
  ...
  return function ({ characterList }) {
    // instead of import in top of file, require here to avoid module reference error
    const { CharacterInfo } = require('../data/Store');
    let index = 0;

    for (let i = 0; i < characterList.length; i++) {
      const character = characterList[i];
      const characterType = CharacterInfo.get(character.no).type;
