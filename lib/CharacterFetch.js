...
function fetchFromJapan(number) {
  return coroutine(function * () {
    ...
    const res = yield fetch('http://onepiece-treasurecruise.com/c-' + number, {
      method: 'GET',
      timeout: 30 * 1000
    });
...
function fetchFromTaiwan(number) {
  return coroutine(function * () {
    ...
    const res = yield fetch('http://line-optc.com/tw/c-' + number, {
        method: 'GET',
        timeout: 30 * 1000
    });
...
export default function fetchCharacter(number) {
  return coroutine(function * () {
    ...
    let character = yield fetchFromJapan(number);

    try {
      const taiwan = yield fetchFromTaiwan(number);
