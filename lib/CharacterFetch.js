...
async function fetchFromTaiwan(number) {
  ...
  // Workaround:
  // There is too much trouble in the Taiwan official website,
  // so now we only fetch the essential fields: name, special ability, and captain effect.
  const skill   = $('#left table').eq(number < 437 ? 3 : 4).find('td');
  const captain = $('#left table').eq(number < 437 ? 4 : 5).find('td');
